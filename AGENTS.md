...
for the full plan): application dotfiles previously symlinked in by
`nix/modules/**` are now also expressed as a chezmoi source state under
`chezmoi/`, so `chezmoi apply` can materialize them independently of nix.
`nix/` itself is untouched and still works as the live fallback (see its own
section above) - the original per-tool trees at repo root (`nvim/`,
`wezterm/`, `fish/`, etc.) are still there and still what `nix/modules/**`
symlinks from; `chezmoi/dot_config/**` etc. are separate copies, restructured
into chezmoi's naming convention, not the same files relocated. Keep both in
sync by hand until a later, separate cutover phase removes the nix side.

- **`.chezmoiroot` (repo root) contains the literal text `chezmoi`** - this
  repo's root also holds non-dotfile content (`nix/`, `README.md`, the git
...
- **`chezmoi/.chezmoiignore.tmpl` gates `wezterm/`, `sway/`, `waybar/` out of
  ...
  `nix/hosts/{laptop,server,wsl}/home.nix`'s own imports split - keep both
