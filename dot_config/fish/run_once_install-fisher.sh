...
# (functions/fisher.fish, completions/fisher.fish) that the sibling
# nix-config repo's modules/dev/fish.nix used to fetch via a flake input
# (github:jorgebucaran/fisher, flake = false). Now that fish is
# chezmoi-owned, this run_once_ script fetches the same two files directly
# from upstream, matching the curl-install pattern used elsewhere in this
# repo for tools with no nixpkgs entry (see nix-config's
# modules/dev/agent-cli-tools.nix, read-only reference - not touched by
