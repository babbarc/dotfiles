...
local keys = {
   ...
   { key = 'y',   mods = mod.SUPER, action = act.SpawnCommandInNewTab { args = { 'yazi' } } },
   {
      key = 'F6',
      mods = 'NONE',
   ...
   -- panes: scroll pane
   {
      ...
      action = wezterm.action_callback(function(win, pane)
         ...
         if proc:find('n?vim') then
            win:perform_action(act.SendKey({ key = 'i', mods = mod.SUPER }), pane)
         else
            win:perform_action(act.ScrollByLine(-5), pane)
         end
      end),
   },
